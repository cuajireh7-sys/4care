...
document.addEventListener('DOMContentLoaded', function() {
                ...
                barangayPieChart = new Chart(barangayPieChartCtx, {
                    ...
                    options: {
                        ...
                        plugins: {
                            ...
                            title: {
                                display: true,
                                text: 'Patients per Barangay (545-551)',
                                color: '#222',
                            ...
                            tooltip: {
                                callbacks: {
                                    label: (ctx) => {
                                        ...
                                        return `${labelWithPrefix}: ${count} patients (${percentage}%)`;
                                    }
                                }
                            }
                        },
    ...
    const sectionsToHideSidebar = ['appointments', 'appointments-patient', 'appointments-doctor', 'doctors', 'patients', 'inventory', 'activity-log', 'admin-settings', 'admin-editing', 'report-generation'];
    ...
    // Function to populate barangay dropdown with all options 545-551
    function populateBarangayDropdown() {
        // Always clear and add all barangays 545-551
        barangaySelect.innerHTML = '<option value="all">All Barangays</option>';
        
        // Add all barangays 545-551
        const barangayNumbers = [545, 546, 547, 548, 549, 550, 551];
        for (let i = 0; i < barangayNumbers.length; i++) {
            const option = document.createElement('option');
            option.value = barangayNumbers[i].toString();
            option.textContent = barangayNumbers[i].toString();
            barangaySelect.appendChild(option);
        }
        
        console.log('Barangay dropdown populated with barangays 545-551');
    ...
    let inventoryItems = [];
    // Simple minimal toast (top-right), text only, auto-hides
        ...
        document.querySelectorAll('.edit-inventory-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                ...
                if (itemToEdit) {
                    ...
                    // Fallback: try numeric coercion of ids
                    const alt = inventoryItems.find(item => (parseInt(item.id) === itemId));
                    if (alt) openEditInventoryItemModal(alt);
                }
            });
        });
    ...
    function openEditInventoryItemModal(itemData) {
        ...
        // Store id on the form as a reliable fallback
        try { editInventoryItemForm.dataset.itemId = String(itemData.id); } catch (_) {}
        document.getElementById('editItemName').value = itemData.name || '';
        document.getElementById('editItemQuantity').value = parseInt(itemData.quantity || 0);
        document.getElementById('editItemUnit').value = itemData.unit || '';
        document.getElementById('editItemSupplier').value = itemData.supplier || '';
        document.getElementById('editItemExpiryDate').value = (itemData.expiry_date || itemData.expiryDate || '').substring(0,10);
    ...
    if (editInventoryItemForm) {
        editInventoryItemForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            let currentId = parseInt(document.getElementById('editItemId').value, 10);
            if (!Number.isFinite(currentId) || currentId <= 0) {
                const fallback = parseInt((editInventoryItemForm && editInventoryItemForm.dataset && editInventoryItemForm.dataset.itemId) || '', 10);
                if (Number.isFinite(fallback) && fallback > 0) currentId = fallback;
            }
            const existing = inventoryItems.find(i => (i.id == currentId));
            const updatedItem = {
                id: currentId,
                name: (document.getElementById('editItemName').value || (existing ? existing.name : '')).trim(),
                quantity: parseInt(document.getElementById('editItemQuantity').value || (existing ? existing.quantity : 0), 10),
                unit: (document.getElementById('editItemUnit').value || (existing ? existing.unit : '')).trim(),
                supplier: document.getElementById('editItemSupplier').value || (existing ? existing.supplier : ''),
                expiry_date: document.getElementById('editItemExpiryDate').value || (existing ? (existing.expiry_date || existing.expiryDate || '') : ''),
                location: document.getElementById('editItemLocation').value || (existing ? existing.location : '')
            };

            if (!Number.isFinite(updatedItem.id) || updatedItem.id <= 0) {
                alert('Update failed: Missing item id. Please reopen the item and try again.');
                return;
            }
            if (!updatedItem.name || !updatedItem.unit) {
                alert('Update failed: Name and unit are required.');
                return;
            }

            try {
                const res = await fetch('/4care/Admin-Dash/Back-end/api/update-inventory.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (!res.ok || json.success === false) {
                    throw new Error((json && json.message) ? json.message : `Update failed (${res.status})`);
                }
                loadInventory();
                loadInventoryStats();
