...
if (($data['action'] ?? '') === 'login') {
    ...
    try {
        ...
        $stmt = $pdo->prepare('SELECT id, firstName, lastName, email, password FROM patient_signup WHERE email = ? LIMIT 1');
...
try {
    ...
    // Ensure connection works and use existing table `patient_signup`
    $check = $pdo->query('SELECT 1')->fetchColumn();

    // Reject duplicate email
    $stmt = $pdo->prepare('SELECT id FROM patient_signup WHERE email = ? LIMIT 1');
    $stmt->execute([$email]);
    if ($stmt->fetch()) {
    ...
    $insert = $pdo->prepare('INSERT INTO patient_signup (firstName, lastName, email, password) VALUES (?, ?, ?, ?)');
